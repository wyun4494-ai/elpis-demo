...
module.exports = (app) => {
  ...
  return class PopularProductRecommendService {
    /**
     * 格式化日期时间
     ...
     */
    async getPopularProductRecommendList(params) {
      const {
        page = 1,
        pageSize = 10,
        product_name,
      ...
      // 构建查询
      let query = app.database('t_popular_product_recommend as ppr')
        .leftJoin('t_product as p', 'ppr.product_id', 'p.product_id')
        .where('ppr.status', 1)
      ...
      if (recommend_status !== undefined && recommend_status !== null && recommend_status !== '') {
        query = query.where('ppr.recommend_status', recommend_status)
      }

      // 排序
      const allowedSortFields = ['sort_order', 'create_time', 'view_count']
      let orderByField = 'ppr.sort_order'
      let orderByDirection = 'asc'

      if (sortField && allowedSortFields.includes(sortField)) {
        orderByField = `ppr.${sortField}`
      ...
      for (const item of list) {
        ...
        if (!item.manual_control || item.manual_control === 0) {
          ...
          if (calculatedStatus !== item.recommend_status) {
            await app.database('t_popular_product_recommend')
     ...
     */
    async getPopularProductRecommendDetail(recommendId) {
      const recommend = await app.database('t_popular_product_recommend as ppr')
        .leftJoin('t_product as p', 'ppr.product_id', 'p.product_id')
        .where('ppr.recommend_id', recommendId)
        .where('ppr.status', 1)
        .where(function() {
          this.whereNull('p.product_id').orWhere('p.status', 1)
        })
     ...
     */
    async createPopularProductRecommend(data) {
      const {
        product_id,
        recommend_title,
        recommend_desc,
      ...
      // 检查商品是否已被推荐
      const existingRecommend = await app.database('t_popular_product_recommend')
        .where('product_id', product_id)
        .where('status', 1)
        .first()
      ...
      // 插入数据
      await app.database('t_popular_product_recommend').insert({
        recommend_id,
        product_id,
        recommend_title,
        recommend_desc,
     ...
     */
    async updatePopularProductRecommend(recommendId, data) {
      const {
        recommend_title,
        recommend_desc,
        recommend_image,
      ...
      // 检查推荐是否存在
      const recommend = await app.database('t_popular_product_recommend')
        .where('recommend_id', recommendId)
        .where('status', 1)
        .first()
     ...
     * @returns {Promise<void>}
     */
    async deletePopularProductRecommend(recommendId) {
      const recommend = await app.database('t_popular_product_recommend')
        .where('recommend_id', recommendId)
        .where('status', 1)
        .first()
    ...
    async getAvailableProducts(params = {}) {
      ...
      // 获取所有已推荐的商品ID
      const recommendedProductIds = await app.database('t_popular_product_recommend')
        .where('status', 1)
        .pluck('product_id')
